...
Final source-name review preserves lowercase bare-name intent and all distinctive tokens beyond the old eight-token cap. Both have real retrieval regressions. Optimized browser history diagnostics showed coherent ZIP URL history after settled navigation; the initial harness advanced during navigation and sometimes issued an extra Back. The final harness waits for the selected ZIP URL and tests exactly one Back/Forward. Earlier failures remain recorded.

The directory source also contains inconsistent recorded city/state fields for some ZIP-equal listings. This ticket certifies recorded ZIP equality only. It does not certify geocoding, company relevance of every legacy listing, insurance product capability, or service territory; those source-quality issues remain open.

The complete optimized existing-session run reproduced a Next client-navigation history bounce after directory refresh, despite waiting for the correct selected ZIP URL. A plain same-origin document link now performs this cross-surface handoff, preserving its bounded URL and browser history without a client router transition. The isolated diagnostic had passed but was insufficient; the full-run failures are retained and acceptance is rerun after this repair.

Vercel Agent Review completed successfully on 19ccca8 (automated review a273f8200303caaa54142ba3b2ff2530) with one additional suggestion: unmapped source kinds could throw from classOf during exact NPN lookup. A bounded read-only source query for non-null NPN in insurance_group/carrier/consumer_brand returned no rows in the current window. Three synthetic source-kind tests nevertheless reproduced the exception (60 pass / 3 fail). The repair returns PARTIAL/CAPABILITY_LIMITATION before identity projection or credential/appointment/Marketplace joins, preserves the exact NPN, and does not invent an agency/insurer class or a NO_MATCH. All 63 checks pass afterward. No current business association was changed.
