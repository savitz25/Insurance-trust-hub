...
const origin = process.env.ITH_ORIGIN || 'http://127.0.0.1:3015';
...
async function measure(page) {
  return page.evaluate(() => {
    ...
    const bar = document.body.innerText.slice(0, 400).includes('Ask Trust Hub network');
    const r = (el) => {
      if (!el) return null;
    ...
    return {
      ...
      switchHub: r(sw),
      menu: r(menu),
      stackedBarPresent: bar && (header ? header.getBoundingClientRect().y > 20 : false),
...
ok(!d.stackedBarPresent, 'no stacked Ask network bar');
...
ok(t1024.header && Math.abs(t1024.header.h - 69) <= 1, `1024 header ${t1024.header?.h} = 69`);
ok(t1024.switchHub && t1024.switchHub.h === 44, `1024 Switch Hub visible`);
