...
const origin = process.env.ITH_ORIGIN || 'https://www.insurancetrusthub.com';
...
async function measure(page) {
  return page.evaluate(() => {
    ...
    const networkBar = [...document.querySelectorAll('div, nav')].find((el) =>
...
try {
  ...
  const hh = Math.min((report.desktop1440.stackedChrome || 140) + 8, 200);
  await p.screenshot({
  ...
  const mh = Math.min((report.mobile390.stackedChrome || 140) + 8, 200);
  await mp.screenshot({
