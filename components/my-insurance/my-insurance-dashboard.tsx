...
export function MyInsuranceDashboard({ initial }: Props) {
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-[#E0F2FE] bg-white px-4 py-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Research workspace only â€” tools still work without signing in. Guest saves on this
          ...
          <Button
            ...
            onClick={async () => {
              await signOutAction();
              toast.message('Signed out â€” your research stays on this device');
              router.refresh();
            }}
          >
      ...
      {/* Marketplace research wallet (guest local + optional cloud) */}
      <ResearchWalletPanel />

      {/* Cloud extras (signed-in only) â€” additive, never a replacement for empty shortlist */}
      {user ? (
    ...
    <section>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="text-sm text-slate-500">
          {loadingBasket && items.length === 0 ? 'â€¦' : items.length}
        </span>
      </div>
      {items.length === 0 ? (
        <Card className="border-dashed">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[#E0F2FE] bg-[#E0F2FE]/50 px-4 py-3">
            <div>
              ...
              <p className="text-xs text-slate-500">
                ...
                {basket?.updated_at
                  ? ` Â· Updated ${new Date(basket.updated_at).toLocaleString()}`
                  : ''}
              </p>
            </div>
