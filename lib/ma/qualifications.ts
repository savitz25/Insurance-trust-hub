/**
 * Map Massachusetts DOI agency list / line strings → shared specialty tags.
...
export function classifyMaLine(raw: string): LoaCapability {
  const s = (raw ?? '').trim();
  if (!s) return 'other';
  ...
  if (/agenc/i.test(s)) return 'agency';
  return 'other';
}

export function classifyMaStrings(parts: string[]): LoaCapability[] {
  const set = new Set<LoaCapability>();
  for (const p of parts) {
    const cap = classifyMaLine(p);
...
export function maCapabilitiesToSpecialties(caps: LoaCapability[]): Specialty[] {
  return capabilitiesToSpecialties(caps, 'business');
}

export function maCapabilitiesToInsuranceTypes(caps: LoaCapability[]): InsuranceType[] {
