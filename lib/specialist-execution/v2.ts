...
export async function executeSpecialistV2(req: SpecialistRequest): Promise<{ status: number; body: SpecialistEnvelope }> {
  ...
  try {
    ...
    if (state === 'UNSUPPORTED_CAPABILITY' || state === 'PUBLICATION_RESTRICTED' || state === 'INVALID_QUERY') out.error = { code: state==='INVALID_QUERY'?'invalid_query':state==='PUBLICATION_RESTRICTED'?'producer_publication_restricted':/does not rank/.test(v1.parsed.query.failReason??'')?'ranking_not_supported':/service territory/.test(v1.parsed.query.failReason??'')?'service_territory_not_supported':v1.parsed.query.conditions?.some((c)=>c.meaning==='requested insurance product')?'unresolved_product':v1.parsed.query.entityClass==='insurer'?'legal_insurer_state_cohort_unavailable':v1.parsed.query.refinement==='class'?'entity_class_clarification_required':'unsupported_capability', message: v1.parsed.query.failReason ?? 'This query is not supported.', alternatives: v1.parsed.query.alternatives };
