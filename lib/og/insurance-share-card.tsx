...
export const INSURANCE_SOCIAL_CARD_REVISION = '20260903';
export const INSURANCE_SOCIAL_SAFE_AREA = { width: 820, height: 520 };
export const INSURANCE_SOCIAL_TEXT_LIMITS = {
  titleMaxLines: 2,
...
const FALLBACK_ALT = 'Insurance Trust Hub ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Independent Insurance Research';
const ORANGE = '#0284C7';
...
export function getInsuranceSocialTextLayout(kind: InsuranceShareCardModel['kind'], title: string): TextLayout {
...
function boundedSupportingText(value: string): string {
  ...
  return normalized.length <= INSURANCE_SOCIAL_TEXT_LIMITS.supportingMaxCharacters
    ? normalized
    : `${normalized.slice(0, INSURANCE_SOCIAL_TEXT_LIMITS.supportingMaxCharacters - 1).trimEnd()}...`;
}
...
function Nodes({ side }: { side: 'left' | 'right' }) {
  ...
  return <div style={{ position: 'absolute', inset: 0, display: 'flex' }}>
    {points.map(([x, y], index) => <div key={`${side}-${index}`} style={{ position: 'absolute', left: x, top: y, width: index % 3 === 0 ? 8 : 5, height: index % 3 === 0 ? 8 : 5, borderRadius: 12, background: index % 3 === 0 ? ORANGE : 'rgba(255,255,255,.34)', boxShadow: index % 3 === 0 ? '0 0 22px rgba(2,132,199,.9)' : 'none' }} />)}
    {points.slice(0, -1).map(([x, y], index) => { const [nextX, nextY] = points[index + 1]; const width = Math.hypot(nextX - x, nextY - y); const angle = Math.atan2(nextY - y, nextX - x) * 180 / Math.PI; return <div key={`line-${side}-${index}`} style={{ position: 'absolute', left: x, top: y, width, height: 1, background: 'rgba(2,132,199,.18)', transformOrigin: '0 0', transform: `rotate(${angle}deg)` }} />; })}
  </div>;
}

function Bracket({ side }: { side: 'left' | 'right' }) {
  return <div style={{ position: 'absolute', top: 48, [side]: 156, width: 74, height: 502, borderTop: '2px solid rgba(255,255,255,.35)', borderBottom: '2px solid rgba(255,255,255,.35)', [side === 'left' ? 'borderLeft' : 'borderRight']: `3px solid ${ORANGE}`, borderRadius: side === 'left' ? '52px 0 0 52px' : '0 52px 52px 0', opacity: 0.8, boxShadow: '0 0 28px rgba(2,132,199,.22)' }} />;
}

function Center({ children }: { children: ReactNode }) {
  return <div style={{ position: 'relative', width: INSURANCE_SOCIAL_SAFE_AREA.width, height: INSURANCE_SOCIAL_SAFE_AREA.height, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
...
function InsuranceShareInner({ model }: { model: InsuranceShareCardModel }) {
  const contextual = model.kind !== 'fallback';
  const layout = contextual ? getInsuranceSocialTextLayout(model.kind, model.title) : null;
  return <Center>
    <div style={{ fontFamily: 'Georgia, serif', fontSize: 76, lineHeight: 1, letterSpacing: '-2px', color: '#fff' }}>Insurance Trust Hub</div>
    <div style={{ width: 760, marginTop: 24, display: 'flex', justifyContent: 'center', fontSize: contextual ? 17 : 25, fontWeight: 600, letterSpacing: contextual ? '5px' : '4px', textTransform: 'uppercase', color: contextual ? ORANGE : 'rgba(255,255,255,.9)' }}>{contextual ? boundedSupportingText(model.eyebrow ?? '') : 'Independent Insurance Research'}</div>
    ...
    <div style={{ marginTop: 22, fontSize: 24, letterSpacing: '4px', color: 'rgba(255,255,255,.9)' }}>insurancetrusthub.com</div>
  </Center>;
}

export function renderInsuranceShareImage(model: InsuranceShareCardModel) {
  return new ImageResponse(<div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative', color: '#fff', fontFamily: 'Arial, sans-serif', background: 'radial-gradient(circle at 17% 48%, rgba(2,132,199,.18), transparent 32%), radial-gradient(circle at 84% 45%, rgba(2,132,199,.12), transparent 30%), linear-gradient(135deg, #020b1d, #06182c 54%, #020817)' }}><Nodes side="left" /><Nodes side="right" /><Bracket side="left" /><Bracket side="right" /><InsuranceShareInner model={model} /></div>, { ...INSURANCE_OG_SIZE, headers: { 'Cache-Control': 'public, max-age=0, s-maxage=31536000, immutable', 'X-TrustHub-Card-Revision': INSURANCE_SOCIAL_CARD_REVISION } });
}

export function renderInsuranceFallbackImage() { return renderInsuranceShareImage({ kind: 'fallback', eyebrow: '', title: 'Independent Insurance Research' } as InsuranceShareCardModel); }
export { FALLBACK_ALT as INSURANCE_OG_FALLBACK_ALT };
