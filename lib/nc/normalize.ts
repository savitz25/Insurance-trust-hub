/**
 * Phase 13 — normalize NC DOI / SBS agency / business-entity rows.
...
export type NormalizedNcProducer = {
  entityType: 'business';
  ...
  state: string;
  zip: string | null;
  launchMarketId: NcLaunchMarketId | null;
  identityKey: string;
  skipReason?: string;
};
...
function emptySkip(reason: string, licenseNumber = ''): NormalizedNcProducer {
  return {
    entityType: 'business',
    licenseNumber,
    npn: null,
...
export function normalizeNcAgencyRow(row: Record<string, string>): NormalizedNcProducer {
  ...
  const orgType =
    pick(row, ['Org Type', 'Organization Type', 'Business Type', 'agency_type']) || null;
  const licenseType = pick(row, ['License Type', 'license_type', 'License Class', 'Class']);
  ...
  const state = (pick(row, ['State', 'state', 'Business State']) || 'NC').toUpperCase().slice(0, 2);
  const zip = normalizeZip(
    pick(row, ['Zip', 'ZIP', 'Postal Code', 'Postal code', 'Zip Code', 'Mailing Zip'])
  );
  const issueDate = parseDate(
    pick(row, ['Issue Date', 'Effective Date', 'license_issue_date', 'Issued', 'Original Issue Date'])
  );
  const expirationDate = parseDate(
    pick(row, ['Expiration Date', 'Expires', 'expiration_date', 'Expiry', 'License Expiration Date'])
  );
  const statusRaw = pick(row, ['Status', 'License Status', 'license_status', 'Active Status']);
  ...
  const market = matchNcLaunchMarket({
    county,
    city: cityRaw,
    zip,
  });
  ...
  const capabilities = classifyNcStrings([...licenseTypes, ...qualifications]);
  ...
  return {
    ...
    identityKey: `ncdoi:biz:${licenseNumber.toUpperCase()}`,
  };
}

export function mergeNcProducers(rows: NormalizedNcProducer[]): NormalizedNcProducer | null {
  ...
  base.capabilities = classifyNcStrings([...base.licenseTypes, ...base.qualifications]);
  const market = matchNcLaunchMarket({
    county: base.county,
    city: base.city,
    zip: base.zip,
  ...
  base.identityKey = `ncdoi:biz:${base.licenseNumber.toUpperCase()}`;
  return base;
}

export function slugifyNcProducer(name: string, licenseNumber: string): string {
  const base = name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '')
