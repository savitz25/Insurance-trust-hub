/**
 * Map Ohio ODI license type / LOA / qualification strings → specialty tags.
...
export function classifyOdiQualification(raw: string): LoaCapability {
  const s = (raw ?? '').trim();
  if (!s) return 'other';
  ...
  if (/\bagency\b/i.test(s) || /business entity/i.test(s) || /\bbus\b/i.test(s)) {
    return 'agency';
  }
  if (/limited lines/i.test(s) || /credit/i.test(s) || /navigator/i.test(s)) {
    return 'other';
  }
  return 'other';
}

export function classifyOdiStrings(parts: string[]): LoaCapability[] {
  const set = new Set<LoaCapability>();
  for (const p of parts) {
    const c = classifyOdiQualification(p);
...
export function odiCapabilitiesToSpecialties(caps: LoaCapability[]): Specialty[] {
  return capabilitiesToSpecialties(caps, 'business');
}

export function odiCapabilitiesToInsuranceTypes(
  caps: LoaCapability[]
): InsuranceType[] {
