...
assert(
  a.featuredFindings.every(
    (f) => f.source && f.limitation && f.doesNotMean.length > 0,
  ),
...
assert(
  a.population.marketplaceObservations.value === 1300108,
  "cms observations",
);
assert(
  a.population.agencyCredentials.value + 1413804 ===
    a.population.credentials.value,
  "credential row identity",
);
assert(
  a.population.agencies.value +
    a.population.persons.value +
    a.population.legalInsurers.value !==
    a.population.agencies.value,
...
assert(
  !a.publicAvailability.publicPersonProfilesEnabled,
  "person profiles disabled",
);

for (const metric of Object.values(a.population)) {
  assert(
    metric.entityClass && metric.cohort && metric.source && metric.limitation,
    `trace ${metric.id}`,
  );
}
...
const page =
  src("app/page.tsx") + src("components/home/insurance-home-intelligence.tsx");
...
assert(homepageModel.includes('href: "/florida"'), "Florida explorer link");
assert(
...
assert(
  page.includes("sourceAsOf") && page.includes("generatedAt"),
  "trace retrieval clock",
);
assert(
...
assert(
  /no paid rankings/i.test(src("app/page.tsx")),
  "editorial no-paid-rankings sentence",
);
...
assert(
  src("lib/seo/metadata.ts").includes("index: true"),
  "index/follow helper intact",
);

const flPage = src("app/florida/page.tsx");
assert(
  flPage.includes("loadFloridaStateView"),
  "florida loader unchanged import",
);
...
assert(
  ...
  "florida fingerprint untouched",
);
assert(FLORIDA_ROUTE === "/florida", "florida route");
assert(FLORIDA_INDEXABLE === true, "florida remains indexable");

assert(
  !src("lib/national/fl-state-intel.ts").includes("INTEL-006"),
  "florida contract file not edited for this task",
);
