/**
 * Phase 13 — import NC DOI / SBS agencies CSV into nc_license_raw + nc_producers.
 *
 *   npm run nc:import -- --file data/nc-raw/agencies.csv --dry-run
 ...
 *
 * Official path: NC DOI SBS Report Generator (business entity / agency only).
 * Save under data/nc-raw/agencies.csv (gitignored). Do not scrape HTML.
 */
...
import {
  ...
  type NormalizedNcProducer,
} from '../../lib/nc/normalize';
import { loadLocalEnv, requireSupabaseOpsEnv } from '../lib/load-local-env';
import { NC_DOI_SBS_URL } from '../../lib/nc/launch-markets';
...
async function main() {
  ...
  if (!file) {
    console.error(
      'Usage: npm run nc:import -- --file data/nc-raw/agencies.csv [--launch-markets-only] [--dry-run] [--limit N]\n' +
        `Ops export: ${NC_DOI_SBS_URL} (business entity / agency list)`
    );
    process.exit(1);
  }
  ...
  if (!existsSync(abs)) {
    ...
    console.error(`Export agencies from: ${NC_DOI_SBS_URL}`);
    console.error('Save as data/nc-raw/agencies.csv (gitignored).');
    process.exit(1);
  }
  ...
  const byLicense = new Map<string, NormalizedNcProducer[]>();

  let batchId: string | null = null;
  if (supabase) {
    const { data, error } = await supabase
      .from('nc_import_batches')
      .insert({
        source_file: abs,
        source_label: 'nc_doi_agencies',
    ...
    if (error || !data) {
      console.error(
        'Failed to create batch (apply migration 20260815120000_north_carolina_doi_inventory.sql):',
        error?.message
      );
      process.exit(1);
    }
  ...
  for await (const rawLine of rl) {
    ...
    if (supabase && batchId) {
      ...
      if (rawBuffer.length >= 200) {
        const { error } = await supabase.from('nc_license_raw').insert(rawBuffer);
        if (error) console.warn('raw flush error', error.message);
        rawBuffer.length = 0;
      }
    }

    const n = normalizeNcAgencyRow(row);
    if (n.skipReason) {
      skipped++;
    ...
    if (n.state !== 'NC') {
      skipped++;
      skipReasons['not_north_carolina'] = (skipReasons['not_north_carolina'] ?? 0) + 1;
      continue;
    }
  ...
  if (supabase && batchId && rawBuffer.length) {
    await supabase.from('nc_license_raw').insert(rawBuffer);
  }

  const merged: NormalizedNcProducer[] = [];
  for (const list of byLicense.values()) {
    const m = mergeNcProducers(list);
    if (!m) continue;
  ...
  if (supabase && batchId) {
    ...
    await supabase.from('nc_import_batches').update({ row_count: rowNum }).eq('id', batchId);
  }
