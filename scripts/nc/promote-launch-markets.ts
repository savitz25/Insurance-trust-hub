/**
 * Phase 13 — promote nc_producers in Wave-1 launch markets → public providers.
 *
 *   npm run nc:promote -- --dry-run
 *   npm run nc:promote -- --market charlotte --limit 50
...
import {
  ...
  evaluateNcPromotionEligibility,
  type NcProducerRow,
} from '../../lib/nc/promote';
...
async function main() {
  ...
  let markets = NC_LAUNCH_MARKETS;
  if (marketArg !== 'all') {
    const m = marketById(marketArg as NcLaunchMarketId);
    if (!m) {
      console.error('--market must be charlotte|triangle|greensboro|wilmington|all');
      process.exit(1);
    }
    markets = [m];
  }
  ...
  for (const market of markets) {
    ...
    for (;;) {
      ...
      if (error) {
        if (/schema cache|does not exist|nc_producers/i.test(error.message || '')) {
          console.log(
            JSON.stringify(
              {
                dryRun,
                note: 'nc_producers not in this database yet — apply 20260815120000_north_carolina_doi_inventory.sql, then import.',
                markets: markets.map((m) => ({
                  id: m.id,
                  displayName: m.displayName,
                  promoteCap: m.promoteCap,
      ...
      for (const row of data as NcProducerRow[]) {
        if (globalLimit > 0 && stats.promoted >= globalLimit) break;
        if (marketPromoted >= market.promoteCap) break;
        stats.scanned++;
        ...
        const result = evaluateNcPromotionEligibility(row, {
          ...
          .upsert(
            {
              ...
              promoted_by: 'phase13_ncdoi_pipeline',
