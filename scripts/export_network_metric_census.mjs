...
async function restCount(base, key, table, query = "", attempts = 4) {
  ...
  for (let i = 0; i < attempts; i += 1) {
    ...
    if (res.ok || res.status === 206 || res.status === 416) {
      ...
      if (!tail || !/^\d+$/.test(tail) || !Number.isSafeInteger(Number(tail))) throw new Error(`Missing exact count for ${table}`);
      return Number(tail);
    }
...
async function main() {
  ...
  const input = {
    ...
    pennsylvaniaSnapshotFingerprint: pub.pennsylvaniaSnapshotFingerprint,
    pennsylvaniaAsOf: pub.pennsylvaniaAsOf,
    pennsylvaniaLicensedCompaniesDistinctNaic: pub.pennsylvaniaLicensedCompaniesDistinctNaic,
    pennsylvaniaEnforcementActions: pub.pennsylvaniaEnforcementActions,
    pennsylvaniaComplaintTableRows: pub.pennsylvaniaComplaintTableRows,
    northCarolinaSnapshotFingerprint: pub.northCarolinaSnapshotFingerprint,
    northCarolinaAsOf: pub.northCarolinaAsOf,
    northCarolinaLicensingActionRows: pub.northCarolinaLicensingActionRows,
    ohioSnapshotFingerprint: pub.ohioSnapshotFingerprint,
    ohioAsOf: pub.ohioAsOf,
    ohioAuthorizedCompaniesDistinctNaic: pub.ohioAuthorizedCompaniesDistinctNaic,
    massachusettsSnapshotFingerprint: pub.massachusettsSnapshotFingerprint,
    massachusettsAsOf: pub.massachusettsAsOf,
    massachusettsLicensedOrApprovedDistinctNaic: pub.massachusettsLicensedOrApprovedDistinctNaic,
    tennesseeSnapshotFingerprint: pub.tennesseeSnapshotFingerprint,
    tennesseeAsOf: pub.tennesseeAsOf,
    tennesseeLicensedCompaniesDistinctNaic: pub.tennesseeLicensedCompaniesDistinctNaic,
  ...
  const candidate = { version: "insurance-metric-census-v1", retrievedAt: new Date().toISOString(), sourceAsOf: null, sourceClockNote: "Read-only sequential exact-count queries; count retrieval is not official source freshness.", liveProductionHost: new URL(base).host, nationalGraph: manifest.nationalGraph };
  const out = process.argv.find((v) => v.startsWith("--out="))?.slice(6);
  if (!out) throw new Error("Specify --out=<review-candidate.json>; canonical datasets are never written");
  writeFileSync(out, `${JSON.stringify(candidate, null, 2)}\n`, "utf8");
  console.log(JSON.stringify({candidate:out, retrievedAt:candidate.retrievedAt, agencies, legalInsurers}));
}
