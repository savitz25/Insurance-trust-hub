...
function baseInput(over: Partial<InsuranceNetworkMetricsInput> = {}): InsuranceNetworkMetricsInput {
  return {
    ...
    newYorkSnapshotFingerprint: 'd4832c3c41c0390d6ffa15755142c6d669e49e6fb0db858f55f761ea894b212e',
    newYorkAsOf: '2026-09-11',
    newYorkDirectoryRows: 1054,
    ...
    pennsylvaniaSnapshotFingerprint: '530c42b9bb39e86bf76160594c5b37a6b105a6742e4a11b7c364a828e2294239',
    pennsylvaniaAsOf: '2026-09-14',
    pennsylvaniaLicensedCompaniesDistinctNaic: 1722,
    pennsylvaniaEnforcementActions: 3232,
    pennsylvaniaComplaintTableRows: 595,
    ...
    publishedStateIntelligencePaths: ['/florida', '/texas', '/new-jersey', '/california', '/washington', '/colorado', '/virginia', '/new-york', '/illinois', '/oregon', '/pennsylvania', '/north-carolina', '/ohio', '/georgia', '/massachusetts', '/tennessee', '/nevada', '/minnesota'],
    ...over,
  };
}
...
describe('missing is not zero; generatedAt is not sourceAsOf', () => {
  it('does not convert unacquired Texas authorized companies or CA admitted universe to zero', () => {
    ...
    assert.equal(metricByKey(m, 'ny_dfs_company_directory_rows').value, 1054);
    assert.equal(metricByKey(m, 'il_directors_order_observations').value, 2896);
    ...
    assert.equal(metricByKey(m, 'published_state_intelligence_pages').value, 18);
    ...
    assert.throws(
      () => computeInsuranceNetworkMetrics(baseInput({ publishedStateIntelligencePaths: ['/florida', '/texas', '/new-jersey', '/california', '/washington', '/colorado', '/virginia', '/new-york', '/illinois', '/oregon'] })),
      /Pennsylvania state intelligence path missing/
    );
    assert.throws(
    ...
    assert.throws(
      () => computeInsuranceNetworkMetrics(baseInput({ publishedStateIntelligencePaths: ['/florida', '/texas', '/new-jersey', '/california', '/washington', '/colorado', '/virginia', '/new-york', '/illinois', '/oregon', '/pennsylvania', '/north-carolina', '/ohio', '/georgia'] })),
      /Massachusetts state intelligence path missing/
    );
    assert.throws(
      () => computeInsuranceNetworkMetrics(baseInput({ massachusettsLicensedOrApprovedDistinctNaic: 6185 })),
      /Massachusetts licensed or approved companies must not equal national legal insurers/
    );
    ...
    assert.throws(
      () => computeInsuranceNetworkMetrics(baseInput({ publishedStateIntelligencePaths: ['/florida', '/texas', '/new-jersey', '/california', '/washington', '/colorado', '/virginia', '/new-york', '/illinois', '/oregon', '/pennsylvania', '/north-carolina', '/ohio', '/georgia', '/massachusetts', '/tennessee'] })),
      /Nevada state intelligence path missing/
    );
    assert.throws(
