/**
 * Phase 24 — promote ms_producers in Wave-1 launch markets → public providers.
 *
 *   npm run ms:promote -- --dry-run
 *   npm run ms:promote -- --market jackson --skip-existing --confirm
 */
...
import {
  ...
  evaluateMsPromotionEligibility,
  type MsProducerRow,
} from '../../lib/ms/promote';
...
async function main() {
  ...
  let markets = MS_LAUNCH_MARKETS;
  if (marketArg !== 'all') {
    const m = marketById(marketArg as MsLaunchMarketId);
    if (!m) {
      console.error(
        '--market must be jackson|gulfport-biloxi|hattiesburg|southaven|tupelo|meridian|all'
      );
      process.exit(1);
    }
    markets = [m];
  ...
  for (const market of markets) {
    ...
    for (;;) {
      ...
      if (error) {
        if (/schema cache|does not exist|ms_producers/i.test(error.message || '')) {
          console.log(
            JSON.stringify(
              {
                dryRun,
                note: 'ms_producers not in this database yet — apply 20260820120000_mississippi_mid_inventory.sql, then import.',
                markets: markets.map((m) => ({
                  id: m.id,
                  displayName: m.displayName,
                  promoteCap: m.promoteCap,
      ...
      for (const row of data as MsProducerRow[]) {
        if (globalLimit > 0 && stats.promoted >= globalLimit) break;
        if (marketPromoted >= market.promoteCap) break;
        stats.scanned++;
        ...
        const result = evaluateMsPromotionEligibility(row, { requireLaunchMarket: true });
        if (!result.ok) {
          bump(result.reason);
          continue;
        }
        ...
        const { error: bridgeErr } = await supabase.from('ms_provider_promotions').upsert(
          {
            producer_id: row.id,
            provider_id: providerId,
            launch_market: market.id,
            promoted_by: 'phase24_msmid_pipeline',
