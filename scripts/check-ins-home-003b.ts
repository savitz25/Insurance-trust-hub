...
assert(
  JSON.stringify(report.includedStates) ===
    JSON.stringify(["FL", "MA", "OH", "TX", "VT"]),
  "five source states",
);
...
assert(
  AGENCY_MULTISTATE.threeToFour === report.buckets.threeToFour,
  "bucket 3-4",
);
assert(
  AGENCY_MULTISTATE.fiveToNine === report.buckets.fiveToNine,
  "bucket 5-9",
);
...
assert(
  ...
  "locked Story #2 title",
);
assert(
  /LICENSED_IN is not SERVES/i.test(f2?.summary || ""),
  "LICENSED_IN ≠ SERVES",
);
assert(
  /attached state credential/i.test(`${f2?.chartCaption} ${f2?.limitation}`),
  "attached state credential evidence",
);
assert(
  !/active agency licenses/i.test(JSON.stringify(f2)),
  "not active licenses",
);
assert(
  !/complete 50-state/i.test(f2?.summary || ""),
  "not a 50-state census in summary",
);
assert(
  f2?.series.every((row) => row.shareOf === AGENCY_MULTISTATE.d2),
  "percentages use D2",
);
...
for (const n of [77887, 61559, 10551, 5760, 74983, 60318, 9590, 5062]) {
  assert(
    !home.includes(String(n)) && !home.includes(n.toLocaleString("en-US")),
    `provisional ${n} absent`,
  );
}
assert(
  home.includes("62,202") || home.includes("62202"),
  "canonical 1-state bucket present",
);

const keyset = readFileSync(
  join(root, "scripts/national/run-ins-home-003.ts"),
  "utf8",
);
...
assert(
  !/\.range\s*\(/.test(
    keyset.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/.*$/gm, ""),
  ),
  "no unordered range pagination in census fetch",
);
assert(/disqualified/i.test(keyset), "unordered range ban documented");

const sql = readFileSync(
  join(root, "scripts/sql/ins-home-003b-lock.sql"),
  "utf8",
);
assert(/^\s*--/m.test(sql) && /GROUP BY/i.test(sql), "SQL GROUP BY lock");
assert(/COUNT\(DISTINCT/i.test(sql), "COUNT DISTINCT in SQL");
assert(
  /BEGIN READ ONLY/.test(
    readFileSync(
      join(root, "scripts/national/run-ins-home-003b-sql.ts"),
      "utf8",
    ),
  ),
...
assert(
  !/\b(INSERT|UPDATE|DELETE|MERGE|CREATE TABLE)\b/.test(sql),
  "lock SQL is SELECT-only",
);
assert(sql.includes("entity_kind = 'agency'"), "person credentials excluded");
assert(
  /^[A-Z]{2}$/.test("FL") && sql.includes("^[A-Z]{2}$"),
  "two-letter jurisdictions",
);
...
assert(
  page.includes('data-intel-event="insurance_intel_trace_number"'),
  "Trace this number",
);
assert(
