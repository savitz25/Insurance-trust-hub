...
assert(
  AGENCY_MULTISTATE.d3 === 109927 && AGENCY_MULTISTATE.d4 === 117354,
  "D3/D4 locked",
);
...
assert(
  a.publicAvailability.publicGraphAgencies === 0,
  "public graph agencies 0",
);
assert(
  a.publicAvailability.publicLegalInsurers === 0,
  "public legal insurers 0",
);
assert(
  a.publicAvailability.publicDirectoryProviders === 170499,
  "directory listings 170,499",
);
...
assert(
  a.featuredFindings[0]?.title ===
    "Insurance is a network, not one company list",
  "story 1 title",
);
assert(
  a.featuredFindings[1]?.title ===
    "Some agencies hold credentials across multiple states",
  "story 2 title",
);
assert(
  a.featuredFindings[2]?.title ===
    "Lines of authority matter — and they are not one national taxonomy yet",
  "story 3 title",
);

const f2 = a.featuredFindings[1];
assert(
  f2?.series.map((row) => row.value).join(",") === "62202,13289,6546,34,0",
  "locked multi-state series",
);
assert(
  f2?.series.every((row) => row.shareOf === 82071),
  "story 2 shareOf is D2",
);

assert(credentialIsAppointment() === false, "credential ≠ appointment");
assert(
  appointmentIsInsurerIdentity() === false,
  "appointment ≠ insurer identity",
);
...
assert(
  geographyIsServiceTerritory() === false,
...
const homepageModel = src("lib/metrics/insurance-home-evidence-inventory.ts");
const zip = src("components/zip-search.tsx");
const pwa = src("components/pwa/insurance-pwa-provider.tsx");
const ui = page + zip;

assert(
  /Separate source-native grains/.test(page),
  "incompatible network grains stay separate",
);
assert(/There is no single/.test(page), "entity classes remain distinct");
assert(
  homepageModel.includes("line_of_authority_observations"),
  "LOA observations remain independent",
);
assert(
  /Search public directory/.test(page),
  "directory CTA is listings, not all licensed agencies",
);
assert(!/Agency directory lookup/i.test(page), "old ZIP heading removed");
assert(
  !/Research licensed agencies/i.test(page),
  "old licensed-agencies CTA removed",
);
assert(
  /public directory listings/i.test(page),
  "ZIP panel says directory listings",
);
assert(
  /not a search of all graph agencies/i.test(ui),
  "ZIP does not claim all graph agencies",
);
...
assert(
  /aria-label="Search public insurance directory listings by ZIP code"/.test(
    zip,
  ),
  "ZIP form labeled",
);
assert(
  /aria-label="ZIP code for public directory listings"/.test(zip),
  "ZIP input labeled",
);
assert(/role="alert"/.test(zip), "ZIP validation alert");
assert(
  homepageModel.includes("public_directory_listings"),
  "directory listing count distinguished from graph agencies",
);
assert(
  homepageModel.includes("appointing_carrier_entities") &&
    homepageModel.includes("licensed_insurance_companies"),
  "legal insurer ≠ entity_kind=carrier",
);
assert(
  homepageModel.includes("appointing_carrier_entities"),
  "carrier-kind count shown as distinct grain",
);
assert(
  /public people profiles[\s\S]*person publishing remains disabled/i.test(page),
  "people not a public directory",
);
...
assert(
  page.includes("sourceAsOf") && page.includes("networkGeneratedAt"),
  "trace source and network clocks",
);
assert(
  !/national Life\/Health\/Property/i.test(ui),
  "no person LOA national product chart",
);
assert(
  !/person LOA (pie|product-line chart)/i.test(ui),
  "person LOA product chart absent",
);
assert(
  !/NATIONAL PERSON PRODUCT-LINE CHART/.test(ui),
  "person product-line chart not present",
);
assert(
  /not collapsed into a fake national Property/.test(
    a.featuredFindings[2]?.summary || "",
  ),
  "story 3 still forbids a fake national pie",
);
assert(
  !/Life, Health, and Property producer/i.test(page),
  "no producer product-line chart UI",
);
assert(
  a.featuredFindings.every((f) => f.id !== "person-loa-product"),
  "no person LOA finding",
);
...
assert(
  /path:\s*["']\/["']/.test(src("app/page.tsx")),
  "homepage metadata path",
);
assert(
  homepageModel.includes('href: "/texas"'),
  "texas intelligence route live",
);
assert(
  homepageModel.includes('href: "/washington"'),
  "washington intelligence route live",
);
assert(
  pwa.includes("box-border") &&
    pwa.includes("max-w-full") &&
    pwa.includes("min-w-0"),
  "PWA banner cannot overflow 390",
);
assert(pwa.includes("shrink-0"), "PWA close control shrink-0");
assert(
  /overflow-x:\s*clip/.test(src("app/globals.css")),
  "root overflow clip safety",
);

const codebook = JSON.parse(
  src("data/codebooks/ins-nat-012-person-loa-v1.json"),
) as {
  nationalPublicChart: boolean;
};
assert(
  codebook.nationalPublicChart === false,
  "person LOA national public chart prohibited",
);
...
assert(
  view.fingerprint === CANONICAL_SNAPSHOT_FINGERPRINT,
  "florida fingerprint constant",
);
assert(view.fingerprint === FL_FP, "Florida fingerprint unchanged");
assert(
  !src("lib/national/fl-state-intel.ts").includes("INS-HOME-002"),
  "florida contract not edited",
);
assert(
  !src("app/florida/page.tsx").includes("INS-HOME-002"),
  "florida page not edited for this task",
);
...
console.log(
  "buckets",
  `${AGENCY_MULTISTATE.one}/${AGENCY_MULTISTATE.two}/${AGENCY_MULTISTATE.threeToFour}/${AGENCY_MULTISTATE.fiveToNine}/${AGENCY_MULTISTATE.tenPlus}`,
);
