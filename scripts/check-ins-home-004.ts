...
const census = JSON.parse(
  readFileSync(join(root, "data/reports/ins-home-004-loa-census.json"), "utf8"),
) as {
  db_writes: number;
  L1: number;
  L2: number;
...
const codebook = JSON.parse(
  readFileSync(
    join(root, "data/codebooks/ins-home-004-agency-loa-v1.json"),
    "utf8",
  ),
) as {
  version: string;
...
const lock = JSON.parse(
  readFileSync(join(root, "data/reports/ins-home-003b-sql-lock.json"), "utf8"),
) as {
  sqlLock: string;
};
...
assert(
  !/unordered/i.test(census.pagination) ||
    /no unordered/i.test(census.pagination),
  "no unordered census",
);
assert(
  census.storyDecision === "INTENTIONALLY_UNCHANGED",
  "Story #3 unchanged",
);
assert(
  codebook.nationalStory === "INTENTIONALLY_UNCHANGED",
  "codebook national story",
);
assert(census.L1 === 69545, "L1 agency LOA rows");
assert(
  census.L2 + census.L3 + census.L4 + census.residual === census.L1,
  "L2+L3+L4+excl = L1",
);
...
for (const row of census.codebook) {
  assert(
    typeof row.raw_label === "string" && row.raw_label.length > 0,
    "raw label",
  );
  assert("raw_code" in row, "raw code field");
  assert(
    ["EXACT", "DEFENSIBLE_COMPOSITE", "SOURCE_SPECIFIC", "UNRESOLVED"].includes(
  ...
  assert(
    row.included_in_national_story === false,
    "not in national family chart",
  );
}
assert(
  !census.codebook.some((r) =>
    /texas_tdi_individual|florida_dfs_individual|vermont_dfr_individual/.test(
      r.source_dataset,
    ),
  ),
  "no person LOA datasets in agency codebook",
);

const extractor = readFileSync(
  join(root, "scripts/national/run-ins-home-004-loa-keyset.py"),
  "utf8",
);
assert(!/\.range\s*\(/.test(extractor), "extractor has no range()");
assert(/order=id/.test(extractor), "ordered keyset");
assert(
  !/cms_marketplace|medicare/i.test(extractor),
  "no CMS/Medicare in extractor",
);
...
assert(
  f2?.title === "Some agencies hold credentials across multiple states",
  "Story #2 title",
);
...
assert(
  ...
  "Story #3 unchanged title",
);
assert(
  f3?.series.map((s) => s.key).join(",") ===
    "texas_tdi_individual,texas_tdi,massachusetts,vermont",
  "Story #3 still source-family rows",
);
assert(
  /not one national taxonomy/i.test(f3?.title || ""),
  "no fake national taxonomy",
);

const page =
  readFileSync(
    join(root, "components/home/insurance-home-intelligence.tsx"),
    "utf8",
  ) + JSON.stringify(a);
const homepageModel = readFileSync(
  join(root, "lib/metrics/insurance-home-evidence-inventory.ts"),
  "utf8",
);
assert(
  !page.includes("77887") && !page.includes("77,887"),
  "obsolete 77887 absent",
);
assert(
  !page.includes("74983") && !page.includes("74,983"),
  "obsolete 74983 absent",
);
...
const snap = JSON.parse(
  readFileSync(
    join(root, "data/reports/fl-ins-006-state-snapshot.json"),
    "utf8",
  ),
);
const ready = JSON.parse(
  readFileSync(
    join(root, "data/reports/fl-ins-006-profile-readiness.json"),
    "utf8",
  ),
);
