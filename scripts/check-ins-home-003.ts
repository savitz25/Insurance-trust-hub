...
assert(
  report.exclusions.agencyCredentialRowsScanned === report.d4,
  "D4 = scanned agency credential rows",
);
assert(
  report.exclusions.excludedUnknownState === 0,
  "no unknown-state counted",
);
assert(
...
assert(
  a.featuredFindings[2]?.id === "lines-of-authority",
  "finding 3 unchanged id",
);
...
assert(
  f2?.series.reduce((n, row) => n + row.value, 0) === AGENCY_MULTISTATE.d2,
  "series sum D2",
);
assert(
...
const pct = f2!.series.reduce(
  (n, row) => n + (100 * row.value) / (row.shareOf || 1),
  0,
);
assert(
  Math.abs(pct - 100) < 0.15,
  `percentage total ~100 got ${pct.toFixed(2)}`,
);
assert(
...
assert(
  !/best|top agent|trust score|safest/i.test(JSON.stringify(f2)),
  "no quality/rank language",
);
