...
def test_repo() -> None:
    ...
    check("new_jersey_route_no_counties", (ROOT / "app" / "new-jersey" / "page.tsx").exists() and not (ROOT / "app" / "new-jersey" / "essex").exists())
    sitemap = (ROOT / "app" / "sitemap.ts").read_text(encoding="utf-8")
    check("sitemap_has_new_jersey_not_counties", ("'/new-jersey'" in sitemap or '"/new-jersey"' in sitemap) and "/new-jersey/" not in sitemap.split("hubs")[0])
