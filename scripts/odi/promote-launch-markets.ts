/**
 * Phase 10 — promote odi_producers in Wave-1 launch markets → public providers.
 *
 *   npm run odi:promote -- --dry-run
 *   npm run odi:promote -- --market columbus --limit 50
 *   npm run odi:promote -- --market all --skip-existing
 */
...
import {
  ...
  evaluateOdiPromotionEligibility,
  type OdiProducerRow,
...
async function main() {
  ...
  let markets = OH_LAUNCH_MARKETS;
  if (marketArg !== 'all') {
    const m = marketById(marketArg as OhLaunchMarketId);
    if (!m) {
      console.error(
        '--market must be columbus|cleveland|cincinnati|toledo|akron|dayton|all'
      );
      process.exit(1);
    }
    markets = [m];
  ...
  for (const market of markets) {
    ...
    for (;;) {
      ...
      for (const row of data as OdiProducerRow[]) {
        if (globalLimit > 0 && stats.promoted >= globalLimit) break;
        if (marketPromoted >= market.promoteCap) break;
        stats.scanned++;
        ...
        const result = evaluateOdiPromotionEligibility(row, {
          ...
          .upsert(
            {
              ...
              promoted_by: 'phase10_odi_pipeline',
